...
public class ExpressElevator extends Elevator{
    ...
    private ArrayList<Passenger> Expresselevator = new ArrayList<>();
    
    //default constructor
    public ExpressElevator() {
         setCurrentFloor(0);
         this.type ="Vip Passenger";
         setMaxCapacity(10000000);
         setNextFloor(-1);
    }
    //parametric constructor
        public ExpressElevator(String type) {
            this.type = type;
        }
       //setter
        ...
        //copy constructor
        public ExpressElevator(ExpressElevator e) {
            this.type = e.type;
    
}

    
    public ArrayList<Passenger> getExpresselevator() {
        return Expresselevator;
    }

   
    public void setExpresselevator(ArrayList<Passenger> Expresselevator) {
        this.Expresselevator = Expresselevator;
    }
    ...
    public void upRequest(ArrayList<ArrayList<Passenger>> Floors) { //parameters have to be the same 
        System.out.println("Express elevator up request");
        System.out.println("Current floor Express elevator: " + getCurrentFloor());
        Passenger p;
        callRequest1 = false;
   ...
   if(callRequest1 == false) {
       downRequest(Floors);
        ...
        System.out.println("Express elevator next request floor: " + getNextFloor());
        ...
        for (int i = getCurrentFloor(); i <= getNextFloor(); i++) { //simulating the elevators moving through the floors
            System.out.println("Express elevator currently at floor number: " + i);
        }
        ...
        System.out.print("Before passengers are removed from floor " + getCurrentFloor() + ": "); //checks passengers before removal from the floor
            ...
            for (int i = 0; i < Expresselevator.size(); i++) { //if startFloor and endFloor equals to -1 and passenger type equals tp "Arrived at destination" it means Passenger reach destination and added back to multidimensional arraylist 
            if(Expresselevator.get(i).getEndFloor() == getCurrentFloor()) {
                Expresselevator.get(i).setType("Arrived at destination");
                Expresselevator.get(i).setStartFloor(-1);
                Expresselevator.get(i).setEndFloor(-1);
                Floors.get(getCurrentFloor()).add(Expresselevator.get(i)); //getting passenger from elevator to the floor
                System.out.println("Passenger: " + Expresselevator.get(i).toString() + " has exited the Express Elevator");
                Expresselevator.remove(Expresselevator.get(i));
            }
        }
        ...
        for (int i = 0; i < Floors.get(getCurrentFloor()).size(); i++) {
            ...
            //and checks max capacity of elevator type
                if(p.getType().equals(type)&& p.getStartFloor() < p.getEndFloor() && Expresselevator.size() < getMaxCapacity()) { 
                    //adds passenger to elevator type
                    Expresselevator.add(p);
                    System.out.println("Passenger: " + p.toString() + " has entered the Express elevator");
        }
        }
        ...
        //removeall method is in the collection class 
            Floors.get(getCurrentFloor()).removeAll(Expresselevator);
            System.out.print("Inside Express elevator: ");
            ...
            if(callRequest1) {  //breaking out of the outer for loop
             ...
             if(callRequest1) {//checking if its true
                 if(!Expresselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Expresselevator.get(0).getEndFloor();//getting the closes end floor request if theres passengers in the elevator hat need to exit
                     for (int i = 0; i < Expresselevator.size(); i++) {
                         if(Expresselevator.get(i).getEndFloor() < min) {
                             min = Expresselevator.get(i).getEndFloor();
                         }    
                 }
                 ...
                 if(!Expresselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Expresselevator.get(0).getEndFloor();//getting the nearest departure request inside the elevator
                     for (int i = 0; i < Expresselevator.size(); i++) {
                         if(Expresselevator.get(i).getEndFloor() < min) {
                             min = Expresselevator.get(i).getEndFloor();
                         }    
    ...
    public void downRequest(ArrayList<ArrayList<Passenger>> Floors) {
        System.out.println("EXPRESS ELEVATOR DOWN REQUEST METHOD");
        System.out.println("Current floor Express Elevator: " + getCurrentFloor());
        ...
        callRequest1 =true;
            ...
            if(!Expresselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Expresselevator.get(0).getEndFloor();//getting the nearest departure request inside the elevator
                     for (int i = 0; i < Expresselevator.size(); i++) {
                         if(Expresselevator.get(i).getEndFloor() > min && Expresselevator.get(i).getEndFloor() > getNextFloor()) {
                             min = Expresselevator.get(i).getEndFloor();
                             setNextFloor(min);
            }
     ...
     if(!Expresselevator.isEmpty()) { //checks if the elevator is not empty
        ...
        for (int i = getCurrentFloor(); i <= getNextFloor(); i++) { //simulating the elevators moving through the floors
            System.out.println("Expresselevator currently at floor number: " + i);
       }
        setCurrentFloor(getNextFloor());
        ...
        for (int i = 0; i < Floors.get(getCurrentFloor()).size(); i++) {
           Passenger pa = Floors.get(getCurrentFloor()).get(i);
            //checks if the types are the same as the elevator and if the startfloor is less than the end floor because we are doing up request
            //and checks max capacity of elevator type
                if(pa.getType().equals(type)&& pa.getStartFloor() > pa.getEndFloor() && Expresselevator.size() < getMaxCapacity()) { 
                    //adds passenger to elevator type
                    Expresselevator.add(pa);
                    System.out.println("Passenger: " + pa.toString() + " has entered the Express elevator");
            }
        }
        // this is removing the elements of passengers in the floors using the removeall method 
        // removes all the elements that are similar in standardelevator to floor and remove it from floors
             ...
             if(callRequest1) {//checking if its true
                 ...
                 if(!Expresselevator.isEmpty()) { //checks if the elevator is not empty
                    ...
                    setNextFloor(min);//min should be greater than elevators current floor
                     ...
                     setCurrentFloor(0);
                     System.out.println("EXPRESS ELEVATOR CALL REQUEST DONE");
