...
public class GlassElevator extends Elevator{
               private String type;
    private ArrayList<Passenger> Glasselevator = new ArrayList<>();
    private boolean callRequest1 = false; 
    //default constructor
    public GlassElevator() {
         setCurrentFloor(0);
         this.type ="Glass Passenger";
         setMaxCapacity(1000000);
         setNextFloor(-1);
    }
    //parametric constructor
        public GlassElevator(String type) {
            this.type = type;
        }
       //setter
        ...
        //copy constructor
        public GlassElevator(GlassElevator g) {
            this.type = g.type;
    
}

   
    public ArrayList<Passenger> getGlasselevator() {
        return Glasselevator;
    }

    
    public void setGlasselevator(ArrayList<Passenger> Glasselevator) {
        this.Glasselevator = Glasselevator;
    }
    ...
    public void upRequest(ArrayList<ArrayList<Passenger>> Floors) { //parameters have to be the same 
        System.out.println("Glass Elevator up request");
        System.out.println("Current floor Glass Elevator: " + getCurrentFloor());
        Passenger p;
        callRequest1 = false;
   ...
   if(callRequest1 == false) {
       downRequest(Floors);
        ...
        System.out.println("Glass Elevator next request floor: " + getNextFloor());
        ...
        for (int i = getCurrentFloor(); i <= getNextFloor(); i++) { //simulating the elevators moving through the floors
            System.out.println("Glass Elevator currently at floor number: " + i);
        }
        ...
        System.out.print("Before passengers are removed from floor " + getCurrentFloor() + ": "); //checks passengers before removal from the floor
            ...
            for (int i = 0; i < Glasselevator.size(); i++) { //if startFloor and endFloor equals to -1 and passenger type equals tp "Arrived at destination" it means Passenger reach destination and added back to multidimensional arraylist 
            if(Glasselevator.get(i).getEndFloor() == getCurrentFloor()) {
                Glasselevator.get(i).setType("Arrived at destination");
                Glasselevator.get(i).setStartFloor(-1);
                Glasselevator.get(i).setEndFloor(-1);
                Floors.get(getCurrentFloor()).add(Glasselevator.get(i)); //getting passenger from elevator to the floor
                System.out.println("Passenger: " + Glasselevator.get(i).toString() + " has exited the Glass Elevator");
                Glasselevator.remove(Glasselevator.get(i));
            }
        }
        ...
        for (int i = 0; i < Floors.get(getCurrentFloor()).size(); i++) {
            ...
            //and checks max capacity of elevator type
                if(p.getType().equals(type)&& p.getStartFloor() < p.getEndFloor() && Glasselevator.size() < getMaxCapacity()) { 
                    //adds passenger to elevator type
                    Glasselevator.add(p);
                    System.out.println("Passenger: " + p.toString() + " has entered the Glass Elevator");
        }
        }
        ...
        //removeall method is in the collection class 
            Floors.get(getCurrentFloor()).removeAll(Glasselevator);
            System.out.print("Inside Glass Elevator: ");
            ...
            if(callRequest1) {  //breaking out of the outer for loop
             ...
             if(callRequest1) {//checking if its true
                 if(!Glasselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Glasselevator.get(0).getEndFloor();//getting the closes end floor request if theres passengers in the elevator hat need to exit
                     for (int i = 0; i < Glasselevator.size(); i++) {
                         if(Glasselevator.get(i).getEndFloor() < min) {
                             min = Glasselevator.get(i).getEndFloor();
                         }    
                 }
                 ...
                 if(!Glasselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Glasselevator.get(0).getEndFloor();//getting the nearest departure request inside the elevator
                     for (int i = 0; i < Glasselevator.size(); i++) {
                         if(Glasselevator.get(i).getEndFloor() < min) {
                             min = Glasselevator.get(i).getEndFloor();
                         }    
    ...
    public void downRequest(ArrayList<ArrayList<Passenger>> Floors) {
        System.out.println("GLASS ELEVATOR DOWN REQUEST METHOD");
        System.out.println("Current floor Glass Elevator: " + getCurrentFloor());
        ...
        callRequest1 =true;
            ...
            if(!Glasselevator.isEmpty()) { //checks if the elevator is not empty
                     int min = Glasselevator.get(0).getEndFloor();//getting the nearest departure request inside the elevator
                     for (int i = 0; i < Glasselevator.size(); i++) {
                         if(Glasselevator.get(i).getEndFloor() > min && Glasselevator.get(i).getEndFloor() > getNextFloor()) {
                             min = Glasselevator.get(i).getEndFloor();
                             setNextFloor(min);
            }
     ...
     if(!Glasselevator.isEmpty()) { //checks if the elevator is not empty
        ...
        System.out.print("Before passengers are removed from floor " + getCurrentFloor() + ": "); //checks passengers before removal from the floor
            ...
            if(Glasselevator.get(i).getEndFloor() == getCurrentFloor()) {
                ...
                Glasselevator.get(i).setEndFloor(-1);
                Floors.get(getCurrentFloor()).add(Glasselevator.get(i)); //getting passenger from elevator to the floor
                System.out.println("Passenger: " + Glasselevator.get(i).toString() + " has exited the Standard Elevator");
                Glasselevator.remove(Glasselevator.get(i));
            }
        }
            System.out.println();
        for (int i = 0; i < Floors.get(getCurrentFloor()).size(); i++) {
           Passenger pa = Floors.get(getCurrentFloor()).get(i);
            //checks if the types are the same as the elevator and if the startfloor is less than the end floor because we are doing up request
            //and checks max capacity of elevator type
                if(pa.getType().equals("Glass Passenger")&& pa.getStartFloor() > pa.getEndFloor() && Glasselevator.size() < getMaxCapacity()) { 
                    //adds passenger to elevator type
                    Glasselevator.add(pa);
                    System.out.println("Passenger: " + pa.toString() + " has entered the Glass Elevator");
            }
        }
        // this is removing the elements of passengers in the floors using the removeall method 
        // removes all the elements that are similar in standardelevator to floor and remove it from floors
             ...
             if(callRequest1) {//checking if its true
                 ...
                 if(!Glasselevator.isEmpty()) { //checks if the elevator is not empty
                    ...
                    setNextFloor(min);//min should be greater than elevators current floor
                     ...
                     setCurrentFloor(0);
                     System.out.println("GLASS ELEVATOR CALL REQUEST DONE");
